        ...
        for (size_t i = 0; i < cachedServerInfo->team_count; i++) {
            ...
            for (size_t j = 0; j < team->player_count; j++) {
                if (team->players[j].playerid == playerid) {
                    ...
                    printf("Player ID %llu set to 0 in cache\n", playerid);
                    pthread_mutex_unlock(&cacheMutex);
                    return;
                }
            }
...
void* monitor_players(void* arg) {
    ...
    for (int i = 0; i < args->count; i++) {
        ...
        } while (!playerStatsArray[i].roundsPlayed&& args->playerid[i] != 0); // 以 roundsPlayed 是否为 0 作为重试标准
    }
        ...
        for (int i = 0; i < args->count; i++) {
            ...
            do {
                ...
                if (stats.roundsPlayed != 0) {
                    ...
                    printf("死亡%d\n", stats.deaths);
                    if (playerStatsArray[i].roundsPlayed < stats.roundsPlayed && (playerStatsArray[i].kills+playerStatsArray[i].deaths)<(stats.kills+stats.deaths)) {
                        increasedCount++;
    ...
    do {
        ...
        if (jiankongflag) {
            ...
            if (operationflag == 1 && attackwinflag)
            {
                ...
                char mapname = fullDetails.serverDetails.maps[mapid].mapPrettyName;
                for (int i = 0; i < 10; ++i) {
